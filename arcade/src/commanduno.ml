...
(** bad input , wrong input format or nonexisting cord*)
...
let parse_helper (layout : char list) play_cards top_card lst =
  ...
  | h :: t -> (
      match h with
      | "play" ->
          if List.length t <> 2 then raise InvalidInput
          else if Unostate.check_valid play_cards top_card then Play t
