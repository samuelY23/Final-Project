...
let parse_helper (layout : char list) turn lst =
  ...
  | h :: t -> (
      match h with
      | "move" -> if (List.length t < 2)then raise InvalidInput else if (Util.is_valid_move t turn layout) then Move t else raise InvalidMove
      | "capture" -> if List.length t < 2 then raise InvalidInput  else if is_valid_move_chain then Capture t else raise InvalidMove (** implement this *)
