...
let rec main () =
  ...
  | x -> (
      ...
      (* change *)
        ...
        if number_pick > 100 then raise (Failure "Invalid Input") else 
        let new_account =
          Account.add
        ...
        let () =
          ...
          if number_pick2 > 100 then raise (Failure "Invalid Input") else 
          let new_account2 =
            Account.add
        ...
        if game_choice = "checkers" then (
          (if !player_number = 1 then (
            player_accounts.data.(0) <- Some (Account.deduct 10 (account_retriever (player_accounts.data.(0))))
          ) else (
            player_accounts.data.(0) <- Some (Account.deduct 10 (account_retriever (player_accounts.data.(0))));
            player_accounts.data.(1) <- Some (Account.deduct 10 (account_retriever (player_accounts.data.(1))))
          ));
